...
public class DisplayDialogueScript : MonoBehaviour {
	private class DialogueClass {
		...
		public DialogueClass (string dialogue) {
			...
			this.text = dialogue;
			this.rect = new Rect(Screen.width / 2, Screen.height * 0.66f, 500, 20);
		}
	}
	...
	private class ChoiceClass {
		...
		public ChoiceClass (string choice, int choiceIndex) {
			if (choiceIndex > 3 || choiceIndex < 1) {
				...
				if (choiceIndex == 1) {
					this.rect.x = Screen.width / 3;
					this.rect.y = Screen.height * 0.75f;
				}
				else if (choiceIndex == 2) {
					this.rect.x = Screen.width * 0.66f;
					this.rect.y = Screen.height * 0.75f;
				}
				else if (choiceIndex == 3) {
					this.rect.x = Screen.width / 2;
	...
	void OnGUI() {
		GUI.Label (dialogue.rect, dialogue.text);
		GUI.Label (choice1.rect, choice1.text);
		GUI.Label (choice2.rect, choice2.text);
		GUI.Label (choice3.rect, choice3.text);
	}
}
